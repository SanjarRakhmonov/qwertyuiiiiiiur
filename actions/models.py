...
class Notification(models.Model):
    ...
    _LIKED_TEMPLATE = u'<a href="/{0}/"><strong>{0}</strong></a> liked your post: <a href="/feeds/{2}/">{3}</a>'
    ...
    _COMMENTED_TEMPLATE = u'<a href="/{0}/"><strong>{0}</strong></a> commented on your post: <a href="/feeds/{2}/">{3}</a>'
    ...
    date = models.DateTimeField(auto_now_add=True)
    feed = models.ForeignKey('posts.Feed', null=True, blank=True)
    ...
    def __str__(self):
        ...
        elif self.notification_type == self.IMAGE_LIKED:
            return self._IMAGE_LIKED_TEMPLATE.format(
                escape(self.from_user.username),
